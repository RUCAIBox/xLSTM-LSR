...
class sLSTM(nn.Module):
    ...
    def forward(self, input_seq, hidden_state=None):
        ...
        for t in range(seq_length):
            x = input_seq[:, t, :]
            new_hidden_state = []
            for j, (lstm, dropout, f_gate, i_gate) in enumerate(zip(self.lstms, self.dropout_layers, self.exp_forget_gates, self.exp_input_gates)):
                if hidden_state[j][0] is None:
                    h, c = lstm(x)
                else:
                    h, c = lstm(x, (hidden_state[j][0], hidden_state[j][1]))
...
class mLSTM(nn.Module):
    ...
    def forward(self, input_seq, hidden_state=None):
        ...
        for t in range(seq_length):
            ...
            new_hidden_state = []
            for j, (lstm, dropout, i_gate, f_gate, o_gate) in enumerate(zip(self.lstms, self.dropout_layers, self.exp_input_gates, self.exp_forget_gates, self.output_gates)):
                if hidden_state[j][0] is None:
                    h, C = lstm(x)
                else:
                    h, C = hidden_state[j]
                ...
                #print("valu:::",values.shape)
                #print("keys:::",keys.shape)
                #print(f.shape)
                #print(C.shape)
                #print(i.shape)
                #print(f * C)
                #print(torch.matmul(values.unsqueeze(2), keys.unsqueeze(1)).squeeze(1).shape)
                C_t = f.unsqueeze(2) * C + i.unsqueeze(2) * torch.matmul(values.unsqueeze(2), keys.unsqueeze(1))
                #.squeeze(1)
                #print("ct::::",C_t.shape)
                #print("que:",queries.shape)
                attn_output = torch.matmul(queries.unsqueeze(1), C_t).squeeze(1)
        ...
        output_seq = torch.stack(output_seq, dim=1)
        #print("out2:",output_seq)
        #output_seq = torch.squeeze(output_seq,-2)
        return output_seq, hidden_state
